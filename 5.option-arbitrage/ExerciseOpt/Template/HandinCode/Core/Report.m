...
myStrikeVec=[8,9,9.5,9.75,10,10.25,10.5,11,12,14];
...
Total=round(value);
Value=round(myUnwindCosts);
ING = {'ING'};
assets = cat(1,ING,GetAllOptionISINs());
table(Position,Value,Total,'RowNames',assets)

% Specific Prints
fprintf('Payments: %d\n',round(sum(value)));
fprintf('Value: %d\n',round(nansum(myUnwindCosts)));
fprintf('Delta: %d\n',DeltaPosition(aBot,myStrikeVec));
fprintf('Gamma: %d\n',GammaPosition(aBot,myStrikeVec));
end